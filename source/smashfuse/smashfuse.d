...
import smashfuse.dbcontroller;
import smashfuse.rss;
...
class SmashFuse {
	...
	void register(URLRouter router)
	{
		...
		router.get(m_subPath ~ "home", &showPostList);
	...
	{
		auto client = HTTP();
		string api_url = "https://www.googleapis.com/youtube/v3/search?part=snippet&q=" ~ query ~ "&maxResults=15&key=AIzaSyBnFg3CxLu-SjZfSgILY2qFXzMZ7BBrPgM";
	...
	protected void showPostList(HTTPServerRequest req, HTTPServerResponse res)
	{
		...
		res.render!("smashfuse.postlist.dt", req, info);
	}
	...
	protected void showPost(HTTPServerRequest req, HTTPServerResponse res)
	{
		...
		res.render!("smashfuse.post.dt", req, info);
	}
	...
	protected HTTPServerRequestDelegate auth(void delegate(HTTPServerRequest, HTTPServerResponse, User[string], User) del)
	{
		return (HTTPServerRequest req, HTTPServerResponse res)
		{
			...
			string username = performBasicAuth(req, res, "SmashFuse admin area", &testauth);
	...
	{
		res.render!("smashfuse.admin.home.dt", req, users, loginUser);
	}
	...
	protected void showConfigList(HTTPServerRequest req, HTTPServerResponse res, User[string] users, User loginUser)
	{
		...
		res.render!("smashfuse.admin.editconfiglist.dt", req, loginUser, configs);
	}
	...
	protected void showConfigEdit(HTTPServerRequest req, HTTPServerResponse res, User[string] users, User loginUser)
	{
		...
		res.render!("smashfuse.admin.editconfig.dt", req, loginUser, globalConfig, config);
	}
	...
	{
		res.render!("smashfuse.admin.edituserlist.dt", req, loginUser, users);
	}
	...
	protected void showUserEdit(HTTPServerRequest req, HTTPServerResponse res, User[string] users, User loginUser)
	{
		...
		res.render!("smashfuse.admin.edituser.dt", req, loginUser, globalConfig, user);
	}
	...
	protected void showEditPosts(HTTPServerRequest req, HTTPServerResponse res, User[string] users, User loginUser)
	{
		...
		//parseJadeFile!("smashfuse.postlist.dt", req, posts, pageNumber, pageCount)(res.bodyWriter);
		res.render!("smashfuse.admin.editpostslist.dt", req, users, loginUser, posts);
	}
	...
	protected void showMakePost(HTTPServerRequest req, HTTPServerResponse res, User[string] users, User loginUser)
	{
		...
		Post post;
		Comment[] comments;
		res.render!("smashfuse.admin.editpost.dt", req, users, loginUser, globalConfig, post, comments);
	}
...
struct SmashFuseHeadlineListConfig {
